...
class Definition:
    ...
    def simplify(self, pos: str):
        if pos not in classifications:
            raise ValueError("Got Part of Speech '{}', which is not in list".format(pos))
...
class DictionaryEntry:
    ...
    def check_tag_type(self, tag: ElementTree.Element, type: str):
        if tag.tag != type:
            raise ValueError("Tag of type {} expected, got {}".format(type, tag.tag))
    ...
    def add_definition(self, tag: ElementTree.Element, last_pos: List[str]):
        ...
        translations = [x.text for x in tag.findall("gloss") if x.attrib["{http://www.w3.org/XML/1998/namespace}lang"] == "eng"]
        
        if len(translations) != 0:
            self.definitions.append(Definition(len(self.definitions) + 1, translations, last_pos))

    def _read_tag(self, tag: ElementTree.Element):
        if tag.tag != "entry":
            raise ValueError("Tag of type Entry expected, got {}".format(tag.tag))
...
def main():
    ...
    for entry in entries:
            entry_root = ElementTree.SubElement(root, "entry", {"title": entry.title})
            
            readings_tag = ElementTree.SubElement(entry_root, "readings")

            for reading in entry.reading_elements:
                reading_tag = ElementTree.SubElement(readings_tag, "reading", {"text": reading.reading})
            ...
            for kanji in entry.kanji_elements:
                kanji_tag = ElementTree.SubElement(kanjis_tag, "form", {"text": kanji.kanji})
            ...
            for definition in entry.definitions:
                definition_tag = ElementTree.SubElement(definitions_tag, "definition")
