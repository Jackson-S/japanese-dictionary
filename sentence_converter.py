...
class WordIndex:
    def __init__(self, parameters: str):
        ...
        self.reading: Optional[str] = self.get_parameter(parameters, ["(", ")"])
    ...
    def get_headword(self, parameters: str):
        ...
        if parameters != "":
            ...
            raise ValueError("Parameters list is blank and contains no headword")
...
def main():
    ...
    for japanese_id, english_id, indices in index_csv:
        if japanese_id in sentence_list and english_id in sentence_list:
            try:
                pair = SentencePair(japanese_id, english_id, indices, sentence_list)
            except ValueError:
                pass
    ...
    for pair in sentence_pairs:
        sentence_node = ElementTree.SubElement(root, "entry", {"jp": pair.jp, "en": pair.en})
